document.addEventListener('DOMContentLoaded', () => {
    ...
    orderButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // Grabs the title and the flavor subtitle (e.g. "PLANT-BASED PROTEIN With Real Pineapple Powder")
            const productTitle = card.querySelector('.product-title').innerText;
            const flavor = card.querySelector('.product-subtitle').innerText;
            
            const orderMessage = encodeURIComponent(`Hi, is "${productTitle} - ${flavor}" available to order right now?`);
            window.open(`https://wa.me/${whatsappNumber}?text=${orderMessage}`, '_blank');
        });
    });

    // ==========================================
    // 3. Tab Switching Logic (Reads from HTML)
    // ==========================================
